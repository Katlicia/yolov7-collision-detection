...
def get_group_boxes(boxes):
    ...
    for box in boxes:
        added = False
        for group in groups:
            if any(iou(box, b) > 0 for b in group):
                group.append(box)
                added = True
                break
        if not added:
    ...
    while groups:
        ...
        for i in range(len(groups)):
            if any(iou(box1, box2) > 0 for box1 in current_group for box2 in groups[i]):
                groups[i].extend(current_group)
                group_merged = True
                break
        if not group_merged:
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):
            ...
            if len(det):
                ...
                grouped_boxes = get_group_boxes(boxes)
                total_people = sum(len(group) for group in grouped_boxes)  # Toplam insan sayısını hesapla
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fourcc', type=str, default='mp4v', help='fourcc codec')
